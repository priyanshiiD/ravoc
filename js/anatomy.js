  ...
  const COMPONENTS = [
    {
      ...
      detail: 'The rotating LiDAR (Light Detection And Ranging) dome fires 1,800 infrared pulses per second at 300 RPM. Each pulse measures distance via time-of-flight, building a precise 360° room map in under 3 seconds. This is the robot\'s primary "eye."',
      color: '#00D4FF',
    ...
    },
    {
      ...
      detail: 'A spring-loaded arc sensor spanning 240° around the front. When physically compressed by contact with an obstacle, it triggers an immediate stop and course-correction. Acts as the last line of defense against impacts.',
      color: '#FF6B35',
    ...
    },
    {
      ...
      detail: 'Paired with the left wheel for independent differential drive. Both wheels include optical encoders that measure rotation to calculate distance traveled, feeding precise odometry data back into the navigation system.',
      color: '#7B4FFF',
    ...
    },
    {
      ...
      detail: 'Two spring-loaded metal contacts at the back of the robot align with the dock station\'s pins. When ROVAC docks, these complete the circuit for 20W fast charging without manual effort.',
      color: '#8AACCC',
      explodeDir: { x: 1.1, y: 1.1 },
    ...
    },
    {
      ...
      detail: 'A dual-material counter-rotating roller brush (bristles + rubber blades) runs the full intake width. It simultaneously agitates carpet fibers to release embedded dirt and scrapes hard floors, channeling all loosened debris into the suction path.',
      color: '#7B4FFF',
      explodeDir: { x: 0, y: 1.4 },
    ...
    },
    {
      ...
      detail: 'The three-arm spinning side brush (counter-clockwise) reaches into corners and along baseboards, areas the circular body cannot directly access, and sweeps debris inward toward the main brush roller and suction inlet.',
      color: '#00E58A',
      explodeDir: { x: -1.1, y: 1.1 },
    ...
    },
    {
      ...
      name: 'Drive Wheel (L)',
      role: 'Differential Movement',
      detail: 'Two independently motorized rubber wheels provide differential steering. By varying the speed and direction of each wheel, ROVAC can spin in place, follow precise arcs, and navigate tight spaces with sub-centimeter accuracy.',
      color: '#7B4FFF',
      explodeDir: { x: -1.25, y: 0 },
      basePos: (R) => ({ x: -R * 0.88, y: 0 }),
      hitTest: (mx, my, cx, cy, R, eT) => {
    ...
    },
    {
      ...
      detail: 'The 500ml sealed dustbin collects all captured debris. Before air exits the system, a HEPA 13 filter traps 99.97% of particles down to 0.3 microns, including fine dust, pollen, and pet dander, ensuring clean exhaust air.',
      color: '#00D4FF',
      explodeDir: { x: -1.1, y: -1.1 },
